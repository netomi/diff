 ...
 * @param <T>
 */
public class OptimizedDiffAlgorithm<T> {
    ...
    public EditScript<T> getEditScript(final List<T> origSequence, final List<T> newSequence,
        ...
        for (Map.Entry<T, EquivalenceClass<T>> entry : map.entrySet()) {
            ...
            if (c.getCountA() > 0 && c.getCountB() > 0 && !c.line.equals("\n")) {
                c.setIndex(index);
                reverseMap[index] = c;
                countA += c.getCountA();
                countB += c.getCountB();
        ...
        if (countA + countB > origSequence.size()) {
            return new MyersAlgorithm<T>().getEditScript(origSequence, newSequence);
        }
        ...
        MyersAlgorithm<Integer> diff = new MyersAlgorithm<>();
        EditScript<Integer> script = diff.getEditScript(listA, listB);
